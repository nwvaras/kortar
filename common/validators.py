...
def validate_ffmpeg_filter_complex(command: str, timeout: int = 10) -> Tuple[bool, Optional[str], str]:
    ...
    if re.search(r"-i\s+(-f\s+null|$|\s+-)", command):
        return False, "Missing input file after -i flag. Please specify a valid input file path.", command
    ...
    try:
        ...
        if result.returncode != 0:
            ...
            logger.error("Validator: FFmpeg command validation failed", pwd=os.getcwd(), error_message=error_msg)
            return False, f"FFmpeg command validation failed with error: {error_msg}", cleaned_command
        
        # Basic syntax validation for filter_complex
        if "-filter_complex" not in cleaned_command and ("overlay=" in cleaned_command or "zoompan=" in cleaned_command):
            return False, "Command should use -filter_complex for the specified filters.", cleaned_command
        ...
        return True, f"Command validation timed out after {timeout} seconds", cleaned_command
