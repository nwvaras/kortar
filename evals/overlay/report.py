...
async def run_overlay_agent(query: str, current_command: str = "ffmpeg -i test.mp4 output.mp4", video_path: str = "test.mp4"):
...
async def main():
    ...
    for golden in dataset.goldens:
        ...
        current_command = golden.additional_metadata.get('current_command', 'ffmpeg -i test.mp4 output.mp4')
        video_path = golden.additional_metadata.get('video_path', 'test.mp4')
        
        # Run the overlay agent
        actual_output = await run_overlay_agent(golden.input, current_command, video_path)
